...
func _on_clickable_area_input_event(viewport, event, shape_idx):
	if event is InputEventMouseButton and event.pressed:
		$improvement_panel.open()


func _on_improvement_panel_scan_time_reduced(scan_time_reduction):
	scan_time -= scan_time_reduction


func _on_improvement_panel_refutation_speed_increased(refutation_speed_increment):
